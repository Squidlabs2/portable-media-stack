...
prompt_first_run_preset() {
  ...
  while true; do
    ...
    case "$answer" in
      1)
        SETUP_PRESET=private
        set_kv MODE tailnet-only
        ;;
      2)
        SETUP_PRESET=funnel
        set_kv MODE tailscale-funnel
        ;;
      3)
        SETUP_PRESET=cloudflare
        set_kv MODE cloudflare-tunnel
        ;;
      4)
        SETUP_PRESET=custom
        return 0
        ;;
      *)
        echo "Invalid selection: $answer. Enter a number from 1 to 4."
        continue
        ;;
    esac

    set_kv ENABLE_SABNZBD false
    set_kv ENABLE_NZBDAV true
    set_kv ENABLE_SEERR true
    set_kv ENABLE_JELLYFIN_INTEL_GPU false
    set_kv AUTO_APPLY_BOOTSTRAP_DATA false
    return 0
  done
}
...
refresh_derived_config_paths() {
  if [ "$(get_value MODE)" = "tailscale-funnel" ]; then
    set_kv TRAEFIK_FUNNEL_CONFIG_DIR "$(get_value CONFIG_ROOT)/traefik-funnel"
  fi
}
...
if [ "$(get_value MODE)" = "traefik-private-dns" ] || [ "$(get_value MODE)" = "traefik-public-dns" ] || { [ "$(get_value MODE)" = "cloudflare-tunnel" ] && [ "$SETUP_PRESET" != cloudflare ]; }; then
